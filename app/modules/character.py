...
from app.types_.dofus.scripts.com.ankamagames.dofus.network.types.game.data.items.effects.ObjectEffectDate import \
    ObjectEffectDate


class Character:
    def __init__(self, objects: list[ObjectItem]) -> None:
        self.objects = [_object for _object in objects if
                        not any([isinstance(_effect, ObjectEffectDate) for _effect in _object.effects])]

    def on_object_added_msg(self, object_: ObjectItem) -> None:
        if not any([isinstance(_effect, ObjectEffectDate) for _effect in object_.effects]):
