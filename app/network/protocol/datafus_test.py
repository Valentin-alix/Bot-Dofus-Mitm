...
class Attribute:
    ...
    def buildSocketType(self):
        self._socket_type = "TypeId<" + str(self._types[0]) + ">" if self._has_type_id and len(
            self._types) < 2 else str(self._types[0])
        for i in range(1, len(self._types)):
            self._socket_type = ("TypeIdVector<" if self._has_type_id else "Vector<") + self._socket_type + "," + str(
                self._types[i]) + ">"
...
if __name__ == "__main__":
    input_folder = os.path.join(Path(__file__).parent.parent.parent.parent, "resources", "code_source")

    main(input_folder, "datafus_test.json")
