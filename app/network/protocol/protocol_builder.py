...
class Attribute:
    ...
    def __init__(self, name, script_type, lines):
        ...
        self._pattern = re.compile(
            rf"output.write(\w+)\(this.{name}|this.{name}\s=\snew\s{script_type}\((\d+),"
        )
        self._type_id_pattern = re.compile(
            rf"output.write(\w+)\([\(]?this.{name}[a-zA-Z [_\]0-9]*[\)]?.getTypeId"
        )
    ...
    def buildSocketType(self):
        self._socket_type = (
            "TypeId<" + str(self._types[0]) + ">"
            if self._has_type_id and len(self._types) < 2
            else str(self._types[0])
        )
        for i in range(1, len(self._types)):
            self._socket_type = (
                ("TypeIdVector<" if self._has_type_id else "Vector<")
                + self._socket_type
                + ","
                + str(self._types[i])
                + ">"
...
class ActionScriptReader:
    ...
    class_pattern = re.compile(
        r"public\sclass\s(\w+)\s(?:extends\s(\w+)\s)?(?:implements\s([\w,\s]+))?"
    )
            ...
            for line in lines:
                ...
                if attribute_match:
                    self.attributes.append(
                        Attribute(
                            attribute_match.group(1), attribute_match.group(2), lines
                        )
                    )
                    continue
...
if __name__ == "__main__":
    source_folder = os.path.join(
        Path(__file__).parent.parent.parent.parent, "resources", "code_source"
    )
    output_file = os.path.join(Path(__file__).parent, "protocol.json")
