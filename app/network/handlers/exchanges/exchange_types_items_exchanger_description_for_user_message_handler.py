...
from app.interfaces.dofus.scripts.com.ankamagames.dofus.network.messages.game.inventory.exchanges.ExchangeTypesItemsExchangerDescriptionForUserMessage import (
...
class ExchangeTypesItemsExchangerDescriptionForUserMessageHandler(
    ...
    def handle(self, bot_info: BotInfo, app_signals: AppSignals) -> None:
        if len(self.itemTypeDescriptions) >= 1:
            lowest_price_item_unity = min(
                [_item.prices[0] for _item in self.itemTypeDescriptions]
            )
            lowest_price_item_ten = min(
                [_item.prices[1] for _item in self.itemTypeDescriptions]
            )
            lowest_price_item_hundred = min(
                [_item.prices[2] for _item in self.itemTypeDescriptions]
            )
            ...
            filtered_price = list(
                filter(
                    lambda elem: elem != 0,
                    [
                        lowest_price_item_unity,
                        lowest_price_item_ten // 10,
                        lowest_price_item_hundred // 100,
                    ],
                )
            )
        ...
        logger.info("Got minimal prices of objects")
